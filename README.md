...
- V3.8 result: **OUT-fence/DQ race was post-latch; its intentional drop added one frame**
- current experiment: **V3.8.1 non-invasive CRTC-sequence window profiler**
...
## V3.8 result and V3.8.1 correction

```bash
bash ~/src/rk3588-hdmi-lowlatency/scripts/pi-debug.sh v381
```

The V3.8 hardware run proved that V4L2 readiness can beat userspace OUT-fence
notification even though the CRTC vblank has already advanced. Its rejected
overlap (`EBUSY`) and intentional capture drop were followed by a two-vblank
31.93 ms completion, adding the frame visible in the high-speed video. V3.8 is
therefore retired.

V3.8.1 runs one uninterrupted 120-second arm. It polls the same readiness race
but queries the authoritative CRTC sequence before classifying it. It never
dequeues the candidate inside the probe, never makes an overlapping commit and
never drops a capture. The ordinary explicit-sync loop remains unchanged.
...
- `docs/V381_RUNBOOK.md` — corrected single-arm profiler and interpretation
